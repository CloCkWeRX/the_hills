...
class TheHillsScraper
  ...
  def enquiry_url
    "https://epathway.thehills.nsw.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquiryLists.aspx?ModuleCode=LAP"
  end
  ...
  def urls
    ...
    (1..number_of_pages).each do |page_no|
      ...
      if page_no > 1
        page = agent.get("http://epathway.thehills.nsw.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquirySummaryView.aspx?PageNumber=#{page_no}")
      end
  ...
  def applications
    urls.map do |url|
      ...
      date_received = extract_field(table.search('tr')[1], "Lodgement Date")
      day, month, year = date_received.split("/").map{|s| s.to_i}
      application_id = extract_field(table.search('tr')[0], "Application Number")

      description = ""
      # Not rendered on the application pages.
      # description = extract_field(table.search('tr')[3], "Proposal").squeeze(" ").strip

      addresses = []
      address = extract_field(table.search('tr')[3], "Formatted Application Location")
